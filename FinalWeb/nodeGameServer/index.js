...
io.on('connection', function(socket){
	...
	socket.on('play', function(data){
		...
		if( clients.length == 0){
			...
			playerSpawnPoints = [];
			data.playerSpawnPoints.forEach(function(_playerSpawnPoit){
				var playerSpawnPoint = {
					position: _playerSpawnPoit.position,
					rotation: _playerSpawnPoit.rotation
		...
		//in your current game, tell you that have joined
		console.log(currentPlayer.name+' emit: play: '+JSON.stringify(currentPlayer));
		socket.emit('play',currentPlayer);
		//in your current game, we need to tell the other players about you
		socket.broadcast.emit('ohter player connected', currentPlayer);
	...
	socket.on('disconnect', function(){
		...
		console.log(currentPlayer.name+ 'bcst: ohter player disconnected' + JSON.stringify(currentPlayer));
		for(var i =0; i<clients.length; i++){
